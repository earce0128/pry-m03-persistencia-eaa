...
//			System.out.println(ex.getClass().getName());
			...
			List.of(ex.getStackTrace()).stream()
					.map(stI -> stI.getClassName() + ":" + stI.getLineNumber() + ", en " + stI.getMethodName())
					.forEach(x -> System.out.println(x));
		} catch (NoSuchElementException ex) {
//			System.out.println(ex.getClass().getName());
			System.out.println("Mensaje:" + ex.getMessage());
			System.out.println("Al parecer la implementación usada " + "no valida parámetros de entrada");
		} catch (Exception ex) {
			System.out.println(ex.getClass().getName());
			System.out.println("Mensaje:" + ex.getMessage());
		}
	...
	private static void testCreacionPcOk_conPcBuilder() {
		...
		Componente miPc = Componente.getPcBuilder().definirId("pc0001").definirDescripcion("Laptop 15000 s300")
				.definirMarcaYmodelo("Dell", "Terminator")
				.agregarDisco("D001", "Disco Seagate", "TechXYZ", "X200", new BigDecimal("1880.00"),
						new BigDecimal("2000.00"), "1TB")
				.agregarMonitor("M001", "Monitor 17 pulgadas", "Sony", "Z9000", new BigDecimal("3200.00"),
						new BigDecimal("6000.00"))
				.agregarTarjetaVideo("C001", "Tarjeta XYZ", "TechBrand", "X200", new BigDecimal("150.00"),
						new BigDecimal("200.00"), "16GB")
				.agregarDisco("D002", "Disco Seagate", "TechXYZJr", "X100", new BigDecimal("1000.00"),
						new BigDecimal("1600.00"), "500GB")
				.build();
		miPc.mostrarCaracteristicas();
	...
	private static void testCreacionPcErroneo_conPcBuilder_DiscosDeMas() {
		...
		Componente miPc = Componente.getPcBuilder().definirId("pc0001").definirDescripcion("Laptop 15000 s300")
				...
				.agregarDisco("D002", "Disco Seagate", "TechXYZJr", "X100", new BigDecimal("1000.00"),
						new BigDecimal("1600.00"), "500GB")
				.agregarDisco("D003", "Disco Xtr 1TB", "Xtr-500", "Xtr", new BigDecimal("2000.00"),
						new BigDecimal("2600.00"), "1TB")
				.build();
		miPc.mostrarCaracteristicas();
	...
	private static void testGenerarCotizacion() {
		...
		Componente disco = Componente.crearDiscoDuro("D-23", "Disco estado sólido", "Seagate", "T-455",
				new BigDecimal(500), new BigDecimal(1000), "2TB");
